...
# Webmail Service Installer
# This script installs the systemd service for the Webmail application

set -e

echo "=========================================="
echo "Webmail Service Installer"
echo "=========================================="
echo ""
...
if [ "$EUID" -ne 0 ]; then 
    ...
    echo "   Usage: sudo ./install_webmail_service.sh"
    exit 1
fi
...
echo "   Webmail Directory: $WEBMAIL_DIR"
...
# Check if webmail directory exists
if [ ! -d "$WEBMAIL_DIR" ]; then
    echo "❌ Error: Webmail directory does not exist: $WEBMAIL_DIR"
    exit 1
fi
...
echo "✅ Installation directories found"
...
chown -R $VIRTUALENV_USER:$VIRTUALENV_GROUP "$WEBMAIL_DIR"
echo "✅ Ownership set to $VIRTUALENV_USER:$VIRTUALENV_GROUP"

# Test webmail app manually
echo "🧪 Testing webmail application..."
cd "$WEBMAIL_DIR"
export $(cat ../.env | grep -v '^#' | xargs)
echo "   Running: python app.py --check"
"$VENV_DIR/bin/python3" -c "
import sys
sys.path.insert(0, '.')
try:
    import app
    print('✅ Webmail application imports successfully')
except Exception as e:
    print(f'❌ Webmail application import failed: {e}')
    sys.exit(1)
"

if [ $? -ne 0 ]; then
    echo "❌ Webmail application test failed"
    exit 1
fi

# Copy service file to systemd directory
echo "📋 Installing systemd service..."
cp "$WEBMAIL_DIR/$SERVICE_FILE" "$SYSTEMD_DIR/$SERVICE_FILE"
...
sleep 2
HEALTH_CHECK=$(curl -s http://localhost:26000/health 2>/dev/null || echo "failed")
if [[ "$HEALTH_CHECK" == *"healthy"* ]]; then
    echo "✅ Health check passed - Webmail is responding"
else
...
echo ""
echo "🔗 Webmail is listening on: http://localhost:26000"
echo "   (Accessible via HTTPS through Caddy)"
echo ""
echo "📝 Usage:"
echo "   Access webmail with: https://yourdomain.com/?email=user@domain.com"
echo "   Example: https://webmail.puntoa.ar/?email=jlvillaronga@puntoa.ar"
echo ""
echo "📝 Configuration:"
echo "   Service file: $SYSTEMD_DIR/$SERVICE_NAME"
echo "   Working dir:  $WEBMAIL_DIR"
